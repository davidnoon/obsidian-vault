      ...
      if (this.settings.groups.length > 0) {
        const groupVCards = iCloudVCards.filter(
          (vCard) => (
            ...
            this.isGroupCard(vCard) && // That are in the selected groups
            parseVCardToJCard(vCard.data).filter((jCard) => jCard.key === "uid").filter(
              (jCard) => this.settings.groups.some(
                (id) => id === jCard.value
              )
            ).length > 0
          )
        );
        const contactUids = groupVCards.flatMap(
          (vCard) => parseVCardToJCard(vCard.data).filter(
            (jCard) => jCard.key === "xAddressbookserverMember"
          ).map(
            (jCard) => jCard.value.replace("urn:uuid:", "")
          )
        );
        iCloudVCards = iCloudVCards.filter(
          (vCard) => contactUids.some(
            (uid) => vCard.data.includes("UID:" + uid)
          )
        );
      }
      iCloudVCards = iCloudVCards.filter(
...
var SettingTab = class extends import_obsidian2.PluginSettingTab {
  constructor(app, plugin, fetchContacts2) {
    super(app, plugin);
    this.fetchContacts = fetchContacts2;
    this.plugin = plugin;
  }
  display() {
    ...
    this.fetchContacts(
      ...
      const groups = contacts.map((iCloudVCard) => parseVCardToJCard(iCloudVCard.data)).filter((jCard) => {
      ...
      if (groups.length > 0) {
        for (const group of groups) {
          ...
          if (fnJCard && !Array.isArray(fnJCard.value) && uidJCard && !Array.isArray(uidJCard.value)) {
            new import_obsidian2.Setting(containerEl).setName(fnJCard.value).addToggle(
              (bool) => bool.setValue(
      ...
      loadingWrapper.remove();
      containerEl.createEl("p", {
        text: "Error loading groups: " + (error instanceof Error ? error.message : String(error))
      });
    });
  }
  ensureStyles() {
